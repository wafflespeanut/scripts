    ...
    while True:                             # For Turbojet/Turbofan engines
        try:
            ...
            if i == 0:
                ...
                w0, we, wpp, wf, re, rf = (z for z in a[i])
                del a[-1]
                continue
            try:
                w0, we, wpp, wf, re, rf = (z for z in a[i])
...
def iterplot():
    ...
    while i < len(data):
        if 'Iteration' in data[i]:
            i += 2
            d.append(float(data[i].split(' ')[2]))
        i += 1
    x = range(1, len(d) + 1)
    plot(x, d, 'ro')
    plot(x, d)
    axis([0, len(x) + 1, min(d) * 0.95, max(d) * 1.1])
    xlabel(r'Iteration $\mathregular{[no\ unit]}$', fontsize = 18)
    ylabel(r'Net Weight $\mathregular{[kg-f]}$', fontsize = 18)
    grid(True)
    show()
