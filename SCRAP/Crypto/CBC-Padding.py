...
def hexed(p):
    return ''.join([format(i, '02x').upper() for i in p])
...
def analyze(File = 'CBC.txt', b = 3):
    ...
    print 'Chosen block:', p
    i = 0
    while True:                     # nth block is padded using PKCS #5. Modify (n-1)th block to find key length!
        p[i] = '00'
        data[b - 2] = ''.join(p)
        print 'Modified block:', p, '\nPosition:', i + 1
        if not test(''.join(data)):
            break
        sleep(1)
        i += 1
    disconnect()
    print 'Last %d bytes are padded!' % (len(p) - i)
    return i                        # Should return 5

def pad(File = 'CBC.txt', b = 3, l = 5):          # Automated brute-force process to find the stuff in (n-1)th block
    if l == None:
        if raw_input('Analyze padding (y/n)? ') == 'y':
            l = analyze()
        else:
            return
    data = read(File, b)
