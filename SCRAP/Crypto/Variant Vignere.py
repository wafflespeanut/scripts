...
def freq():             # Gathering data generated from a known text corpus
    ...
    for i in range(256): dfreq[format(i,'02x').upper()]=0
    for i in fread: m=i.split('\t'); dfreq[format(int(m[0]),'02x').upper()]=float(m[1][:-1])
...
def find():                         # Find the possible plaintext suspects - This is what you should run!
    ksp=analyze(); print '\nWaiting...'; sleep(1.5); data=read(); d=freq(); found=[]
