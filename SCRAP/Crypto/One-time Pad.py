...
fin="CTEXT-2.txt"; # Key in hex byte strings
...
def check(c1,c2):       # XOR stuff and return possible values (eliminating spaces)
    ...
    alp=[]; t1=[c1[i:i+2] for i in range(0,len(c1),2)]; t2=[c2[i:i+2] for i in range(0,len(c2),2)]
...
def ctext(m):           # Rough way to guess the keys
    ...
    while True:
        try:
            ...
            try: ch=format(ord(ch),'02x')           # In case we insert an additional space
            except TypeError: ch=format(ord(ch.split(' ')[-1]),'02x')
            l=collect[p]; k[p]=format(hexor(l,ch),'02x').upper()
        except (KeyboardInterrupt,ValueError): print 'Testing new key...\n'; test(k); print; return k

def test(k=key):        # To manually test whether it's readable English - This is what you should run!
    with open(fin,'r') as file: data=file.readlines()
    data=[i[:-1] if i[-1]=='\n' else i for i in data]
    r=(' '+str(i) if i/10==0 else str(i) for i in range(len(k))); print '  '.join(r),'\n'
    for b,c in enumerate(data):
        s=[]; ch=[c[i:i+2] for i in range(0,len(c),2)]
        for i in range(len(ch)):
            m=hexor(ch[i],k[i]); s.append(' '+chr(m))
        print '  '.join(s),'\t',[b+1]
