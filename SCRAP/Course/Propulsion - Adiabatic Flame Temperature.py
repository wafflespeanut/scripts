    ...
    for i, j in enumerate(s):
        if j in k:
            ...
            if s[i + 1] in n:
                m = ''
                c = 1
                while i + c < len(s) and s[i + c] in n:
                    m += s[i + c]
                    c += 1
                k[j] += int(m)
            else:
...
def cpres(s, hf):       # For constant pressure process
    k1 = count(s)
    c1, c2, c3, c4, c5 = (cp['c'], cp['w'], cp['o'], cp['n'], cp['co'])
    h1, h2, h3 = (h['c'], h['w'], h['co'])
    arr = []
    for i in range(1, 6):
        phi = 0.2 * i
        a, b, c, d = lean(s, phi)
        p = b * c1 + c * c2 + d * c3 + 3.76 * a * c4
        m = round((hf - b * h1 - c * h2 + t0 * p) / p, 4)
        arr.append(m)
    for i in range(1, 6):
        phi = 1 + 0.2 * i
        a, b, c, d, e = rich(s, phi)
        p = b * c1 + c * c2 + d * c3 + 3.76 * a * c4 + e * c5
        m = round((hf - b * h1 - c * h2 - e * h3 + t0 * p) / p, 4)
        arr.append(m)
    return arr
...
def cvol(s, hf):        # For constant volume process
    ...
    for i in range(1, 6):
        ...
        a, b, c, d = lean(s, phi)
        n = a + 1
        p = b * c1 + c * c2 + d * c3 + 3.76 * a * c4 - n * r
        m = round((hf - b * h1 - c * h2 + t0 * p) / p, 4)
        arr.append(m)
    for i in range(1, 6):
        phi = 1 + 0.2 * i
        a, b, c, d, e = rich(s, phi)
        n = a + 1
        p = b * c1 + c * c2 + d * c3 + 3.76 * a * c4 + e * c5 - n * r
        m = round((hf - b * h1 - c * h2 - e * h3 + t0 * p) / p, 4)
        arr.append(m)
    return arr
